...
namespace ifb::eng {
    ...
    gl_hello_quad_create(
        gl_hello_quad& hello_triangle) {

        // compile the pipeline
        bool did_compile = true;
        gl_pipeline_init(hello_triangle.pipeline);
        did_compile &= gl_pipeline_compile_shader_vertex   (hello_triangle.pipeline, GL_HELLO_QUAD_SHADER_VERTEX);
        did_compile &= gl_pipeline_compile_shader_fragment (hello_triangle.pipeline, GL_HELLO_QUAD_SHADER_FRAGMENT);
    ...
    IFB_ENG_INTERNAL void
    gl_hello_quad_render(
        gl_hello_quad& hello_triangle) {

        const byte* data_buffer_vertex = (byte*)GL_HELLO_QUAD_VERTICES;
        const u32*  data_buffer_index  = (u32*)GL_HELLO_QUAD_INDICES;
        const u32   size_buffer_vertex = sizeof(GL_HELLO_QUAD_VERTICES);
        const u32   size_buffer_index  = sizeof(GL_HELLO_QUAD_INDICES);
